...
int		test_cpy(void)
{
	...
	TESTED_NS::map<T1, T2>::iterator it = mp.begin(), ite = mp.end();

	TESTED_NS::map<T1, T2> mp_range(it, --(--ite));
	for (int i = 0; it != ite; ++it)
		it->second = ++i * 5;

	it = mp.begin(); ite = --(--mp.end());
	TESTED_NS::map<T1, T2> mp_copy(mp);
	for (int i = 0; it != ite; ++it)
		it->second = ++i * 7;

	std::cout << "\t-- PART ONE --" << std::endl;
	printSize(mp);  //by 7
	printSize(mp_range); //sum = 7
	printSize(mp_copy); //by 5
